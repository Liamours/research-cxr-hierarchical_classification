...
import src.xai.localization_metrics as loc_metrics
import src.script.run_localization_eval as run_localization_eval
...
from src.train.losses import (
    BAFLLoss, HBCELoss, MaskedBCELoss, SoftHBCELoss, build_loss, train_class_weights,
)
...
def test_soft_hbce_penalty_fires_when_child_positive_parent_negative():
    pairs = [(0, 1)]
    loss_fn = SoftHBCELoss(pairs, lam=1.0)
    logits = torch.tensor([[_logit(0.1), _logit(0.9)]])
    targets = torch.zeros(1, 2)
    mask    = torch.ones(1, 2)
    loss_fn(logits, targets, mask)
...
def test_soft_hbce_active_below_the_hard_threshold():
    """The one property that distinguishes it from HBCELoss: gradient signal
    even when neither probability crosses 0.5 -- e.g. a rare child at 0.2 and
    an unremarkable parent at 0.6 is still inconsistent (child shouldn't be
    positive-leaning while parent isn't), and HBCELoss's hard relus give this
    exactly zero penalty while the soft product does not."""
    pairs = [(0, 1)]
    soft = SoftHBCELoss(pairs, lam=1.0)
    hard = HBCELoss(pairs, lam=1.0)
    logits = torch.tensor([[_logit(0.6), _logit(0.2)]])  # parent 0.6, child 0.2
    targets = torch.zeros(1, 2)
    mask    = torch.ones(1, 2)
    soft(logits, targets, mask)
    hard(logits, targets, mask)
    assert hard._last_penalty == pytest.approx(0.0, abs=1e-6)
    assert soft._last_penalty > 0.0


def test_soft_hbce_penalty_zero_when_edge_masked():
    pairs = [(0, 1)]
    loss_fn = SoftHBCELoss(pairs, lam=1.0)
    logits = torch.tensor([[_logit(0.1), _logit(0.9)]])
    targets = torch.zeros(1, 2)
...
def test_soft_hbce_equals_bce_when_lam_zero():
    pairs = [(0, 1)]
    soft = SoftHBCELoss(pairs, lam=0.0)
    bce  = MaskedBCELoss()
    logits = torch.randn(4, 5)
    targets = (torch.rand(4, 5) > 0.5).float()
    mask    = torch.ones(4, 5)
    assert soft(logits, targets, mask).item() == pytest.approx(
        bce(logits, targets, mask).item(), rel=1e-5
    )


def test_build_loss_resolves_hierarchical_soft():
    loss_fn = build_loss("hierarchical_soft", CANONICAL_LABELS, lam=0.5)
    assert isinstance(loss_fn, SoftHBCELoss)


# ---------------------------------------------------------------------------
# localization_metrics (A9)
# ---------------------------------------------------------------------------

def test_box_iou_and_iobb_perfect_overlap():
    box = (10.0, 10.0, 30.0, 30.0)
    assert loc_metrics.box_iou(box, box) == pytest.approx(1.0)
    assert loc_metrics.box_iobb(box, box) == pytest.approx(1.0)


def test_box_iou_and_iobb_no_overlap():
    a = (0.0, 0.0, 10.0, 10.0)
    b = (20.0, 20.0, 30.0, 30.0)
    assert loc_metrics.box_iou(a, b) == 0.0
    assert loc_metrics.box_iobb(a, b) == 0.0


def test_box_iobb_exceeds_iou_when_pred_extends_outside_gt():
    # pred is NOT a subset of gt here (unlike a pred-fully-inside-gt case,
    # where union collapses to gt_area and IoU == IoBB by construction).
    gt = (0.0, 0.0, 20.0, 20.0)
    pred = (15.0, 15.0, 35.0, 35.0)  # 20x20, overlapping only the gt corner
    assert loc_metrics.box_iobb(pred, gt) == pytest.approx(25 / 400)
    assert loc_metrics.box_iou(pred, gt) < loc_metrics.box_iobb(pred, gt)


def test_pointing_game_hit_and_miss():
    heatmap = np.zeros((50, 50))
    heatmap[25, 25] = 1.0  # argmax at (x=25, y=25)
    assert bool(loc_metrics.pointing_game_hit(heatmap, (20.0, 20.0, 30.0, 30.0)))
    assert not bool(loc_metrics.pointing_game_hit(heatmap, (0.0, 0.0, 5.0, 5.0)))


def test_largest_component_box_isolates_the_bigger_blob():
    # A continuously-varying baseline (not exact zeros) so the 90th-percentile
    # threshold doesn't land on a value shared by a huge flat region -- real
    # Grad-CAM output is bilinear-interpolated and behaves this way, unlike a
    # hard-edged synthetic array of exact 0s and 1s.
    rng = np.random.default_rng(0)
    heatmap = rng.uniform(0.0, 0.05, size=(100, 100))
    heatmap[10:15, 10:15] = 1.0   # small blob, 5x5 = 25 px
    heatmap[50:80, 50:80] = 1.0   # large blob, 30x30 = 900 px
    box = loc_metrics.largest_component_box(heatmap, percentile=90.0)
    x1, y1, x2, y2 = box
    assert x1 >= 45 and y1 >= 45  # picked the large blob, not the small one


def test_average_precision_perfect_ranking_is_one():
    scores = [0.9, 0.8, 0.3, 0.2]
    hits = [True, True, False, False]
    assert loc_metrics.average_precision_at_iou(scores, hits) == pytest.approx(1.0)


def test_average_precision_no_hits_is_zero():
    assert loc_metrics.average_precision_at_iou([0.9, 0.5], [False, False]) == 0.0
...
# ---------------------------------------------------------------------------
# gradcam bucketing (A9b)
# ---------------------------------------------------------------------------

def _box_record(image_id, canonical, iou):
    return {"image_id": image_id, "canonical": canonical, "image_path": f"{image_id}.png",
            "gt_box": (0.0, 0.0, 10.0, 10.0),
            "pred_box": (0.0, 0.0, 10.0, 10.0) if iou > 0 else None,
            "iou": iou, "iobb": iou}


def test_bucket_agreement_patterns_classifies_correctly():
    flat = [_box_record("a", "Atelectasis", 0.0), _box_record("b", "Cardiomegaly", 0.3),
            _box_record("c", "Pneumonia", 0.2), _box_record("d", "Pneumothorax", 0.0)]
    hier = [_box_record("a", "Atelectasis", 0.0), _box_record("b", "Cardiomegaly", 0.4),
            _box_record("c", "Pneumonia", 0.0), _box_record("d", "Pneumothorax", 0.1)]
    buckets = run_localization_eval.bucket_agreement_patterns(flat, hier)
    assert [r["image_id"] for r in buckets["both_miss"]] == ["a"]
    assert [r["image_id"] for r in buckets["both_hit"]] == ["b"]
    assert [r["image_id"] for r in buckets["flat_only"]] == ["c"]
    assert [r["image_id"] for r in buckets["hier_only"]] == ["d"]
    assert sum(len(v) for v in buckets.values()) == 4


def test_select_examples_prefers_different_finding_then_falls_back_to_file_order():
    same_finding = [_box_record("a", "Atelectasis", 0.1), _box_record("b", "Atelectasis", 0.2),
                     _box_record("c", "Cardiomegaly", 0.3)]
    picked = run_localization_eval.select_examples(same_finding, n=2)
    assert [r["image_id"] for r in picked] == ["a", "c"]  # skips b, same finding as a

    all_same = [_box_record("a", "Atelectasis", 0.1), _box_record("b", "Atelectasis", 0.2)]
    picked = run_localization_eval.select_examples(all_same, n=2)
    assert [r["image_id"] for r in picked] == ["a", "b"]  # no different finding -> file order
