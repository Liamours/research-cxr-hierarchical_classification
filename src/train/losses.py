...
SoftHBCELoss — threshold-free hierarchical penalty (R1 pt.2/pt.4, R2 pt.3:
does the negative result hold for a formulation that stays active across the
full probability range, not just past the 0.5 boundary?). Penalty = child_prob
x (1 - parent_prob), the product t-norm (soft-logic AND) relaxation of the
same "child positive AND parent negative" event HBCELoss gates on with hard
relus. Always has gradient signal whenever child_prob > 0 and parent_prob <
1, so it does not depend on either probability crossing 0.5 -- the one
mechanism change vs. HBCELoss, isolating the threshold as the tested variable.
...
class SoftHBCELoss(nn.Module):
    """Threshold-free hierarchical penalty.

    L = L_BCE + lam * mean_over_edges[ child_prob * (1 - parent_prob) ]

    Product t-norm relaxation of HBCELoss's hard indicator: no relu, no 0.5
    cutoff, gradient signal at every (parent_prob, child_prob) pair where an
    edge is masked-in, including low-prevalence children whose probability
    never crosses 0.5 during training.

    edge_pairs / lam: same contract as HBCELoss.
    """
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor,
        ...
        violation = child_prob * (1.0 - parent_prob)
...
def build_loss(label_structure: str, conditions: list[str], lam: float = 0.5,
    ...
    label_structure: 'flat' | 'hierarchical' | 'hierarchical_soft' | 'bafl'.
    """
    if label_structure in ("hierarchical", "hierarchical_soft"):
        ...
        if label_structure == "hierarchical_soft":
            return SoftHBCELoss(pairs, lam=lam)
