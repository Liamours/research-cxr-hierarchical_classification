...
that dataset), so NIH/VinDr non-applicable columns never enter any metric.
A per-class metric is NaN when its masked subset has no applicable samples or
fewer than two classes present (e.g. a label with zero positives in a split);
NaNs are dropped from macro averages. The two-class requirement applies
uniformly across AUROC, F1, and the confusion-matrix family (precision,
recall, specificity, accuracy, balanced accuracy, MCC), so a label excluded
from one macro average is excluded from all of them.
"""
...
def per_class_f1(probs, targets, mask, conditions=CANONICAL_LABELS, threshold=0.5) -> dict:
    ...
    for j, c in enumerate(conditions):
        ...
        if sel.sum() == 0 or len(np.unique(y)) < 2:
            ...
            out[c] = float(f1_score(y, pred, zero_division=0))
    return out
