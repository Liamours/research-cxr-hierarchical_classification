...
def blend_heatmap(image_path, cam, alpha=0.5, image_size=224) -> np.ndarray:
    """Grayscale image blended with a jet-colormapped Grad-CAM heatmap.
    Returns a uint8 (image_size, image_size, 3) array."""
...
def save_overlay(image_path, cam, out_path, alpha=0.5, image_size=224):
    ...
    Image.fromarray(blend_heatmap(image_path, cam, alpha, image_size)).save(out_path)
